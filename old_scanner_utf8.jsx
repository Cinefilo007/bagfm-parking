...
const ScannerAlcabala = () => {
        ...
        const syncSituacion = async () => {
            try {
                ...
                if (!data.identificado) {
                    toast.error('Sesi├│n operativa no v├ílida o relevo pendiente');
                    navigate('/alcabala/dashboard');
                }
            } catch (e) {
                console.error("Fallo sincronizando situaci├│n t├íctica:", e);
                navigate('/alcabala/dashboard');
            }
        };
        syncSituacion();
    ...
    const handleScanSuccess = async (qrToken) => {
        ...
        try {
            ...
            toast.error('Fallo en validaci├│n', { position: 'bottom-center' });
        } finally {
            setCargando(false);
        }
    };
    ...
    const handleCapturarIA = async () => {
        ...
        try {
            ...
            if (modoEscaneoIA === 'cedula') {
                ...
                toast.success('Veh├¡culo capturado');
            }
            
            // Cerrar el modo escaneo tras ├⌐xito
    ...
    return (
        <div className={cn("min-h-screen flex flex-col transition-colors duration-700 pb-24", getBgColor())}>
            ...
            <main className="flex-1 flex flex-col p-3 gap-3 max-w-lg mx-auto w-full">
                ...
                {/* ΓöÇΓöÇ MODO SCANNER QR PRINCIPAL ΓöÇΓöÇ */}
                ...
                {/* ΓöÇΓöÇ MODO RESULTADO / REGISTRO ΓöÇΓöÇ */}
                {resultado && (
                    <div className="flex flex-col gap-3 animate-in zoom-in-95 duration-400">
                        ...
                        <div className={cn(
                            ...
                            <p className="text-[9px] font-bold text-text-muted uppercase tracking-widest mt-1">
                                {resultado.mensaje || "Validaci├│n de Protocolo"}
                            </p>
                        </div>
                        ...
                        <Card className="bg-bg-card/40 backdrop-blur-md border-white/5 rounded-2xl p-4 space-y-5">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    {resultado.requiere_datos_manuales && (
                                        ...
                                        >
                                            <Camera size={12} />
                                            <span className="text-[9px] font-black uppercase italic">Escanear C├⌐dula</span>
                                        </Boton>
                                    )}
                                </div>
                                    ...
                                    <div className="flex-1 space-y-2">
                                        {resultado.requiere_datos_manuales && !resultado.socio ? (
                                            <div className="grid grid-cols-1 gap-2">
                                                ...
                                                <InputManual label="C├⌐dula" value={cedulaManual} onChange={setCedulaManual} />
                                            </div>
                                        ) : (
                                            <>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    {resultado.requiere_datos_manuales && (
                                        ...
                                        >
                                            <Car size={12} />
                                            <span className="text-[9px] font-black uppercase italic">Escanear T├¡tulo</span>
                                        </Boton>
                                    )}
                                </div>
                            ...
                            <div className="pt-2 flex flex-col gap-2">
                                ...
                                <button onClick={() => setResultado(null)} className="h-10 text-[9px] font-black uppercase text-text-muted/60 tracking-[0.3em] bg-white/5 rounded-lg border border-white/5">Abortar Operaci├│n</button>
                            </div>
                        </Card>
                    </div>
                )}
            ...
            {/* ΓöÇΓöÇ OVERLAY DE ESCANEO INMERSIVO (IA) ΓöÇΓöÇ */}
            {modoEscaneoIA && (
                    ...
                    <div className="absolute top-6 left-6 right-6 flex items-center justify-between text-white z-10">
                        <div>
                            ...
                            <h2 className="text-lg font-black uppercase italic leading-none mt-1">
                                Escaneando {modoEscaneoIA === 'cedula' ? 'C├⌐dula' : 'Veh├¡culo'}
                            </h2>
                        </div>
